%function velocity_solve_2d_no_BC.m written 8-24-17 by JTN to solve a 2d velocity
%field based on Tranquillo Murray (1992)

%Note that this version ignores the boundaries for ease of simulation

function v = velocity_solve_2d_no_BC(q,D1X,D1Y,D2X,D2Y,D2XY,D2_bd,n,rho,u,xy_int,bd,total)


      xy_inty = xy_int + total;
      bdy = bd + total;

      LHS = (q(1)+q(2))*D2X(xy_int,2*total) + q(1)/2*D2Y(xy_int,2*total) + q(1)/2*D2X(xy_inty,2*total) +...
