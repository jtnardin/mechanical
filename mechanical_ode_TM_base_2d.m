function yprime = mechanical_ode_TM_base_2d(t,y,q,dx,dy,D1X,D1Y,D2X,D2Y,D2XY,...
    D2_bd_D,D2Xbd_N,D2Ybd_N,A_pos,A_pos_0,A_pos_1,A_neg,A_neg_0,A_neg_1,xy_int,bd,x_bd_0,x_bd_l,...
    y_bd_0,y_bd_l,total,x_int,y_int,xn)
...
%      t
    ...
    %compute A_posx
    Axp_n = A_pos(sigma(n_ep),sigma(n_wp),ve_pos,vw_pos,Vx_pos_loc,1);
    Axp_rho = A_pos(sigma(rho_ep),sigma(rho_wp),ve_pos,vw_pos,Vx_pos_loc,1);
        
    %Construct Aneg (negative matrix)
       
    %compute A_negx
    Axn_n = A_neg(sigma(n_en),sigma(n_wn),ve_neg,vw_neg,Vx_neg_loc,1);
    Axn_rho = A_neg(sigma(rho_en),sigma(rho_wn),ve_neg,vw_neg,Vx_neg_loc,1);
    ...
    %compute A_posy
    Ayp_n = A_pos(sigma(n_np),sigma(n_sp),vn_pos,vs_pos,Vy_pos_loc,xn);
    Ayp_rho = A_pos(sigma(rho_np),sigma(rho_sp),vn_pos,vs_pos,Vy_pos_loc,xn);
    
    
    %Construct Aneg (negative matrix)
    %compute A_posx
    Ayn_n = A_pos(sigma(n_nn),sigma(n_sn),vn_neg,vs_neg,Vy_neg_loc,xn);
    Ayn_rho = A_pos(sigma(rho_nn),sigma(rho_sn),vn_neg,vs_neg,Vy_neg_loc,xn);
    
    
    
    %final derivative calculations
    dndt = (D2X(xy_int,total) + D2Y(xy_int,total)  + D2Xbd_N(x_bd_0,x_bd_l,total)...
